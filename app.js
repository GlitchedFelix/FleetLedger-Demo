...
const LONG_DISTANCE_THRESHOLD = 150;
const LONG_DISTANCE_RATE      = 23;
const PAYOUT_MARKDOWN_PCT     = 15;
const PAYOUT_MARKDOWN_MULT    = 1 - (PAYOUT_MARKDOWN_PCT / 100);
...
const DEMO_EMAIL    = 'demo@fleetledger.app';
const DEMO_PASSWORD = 'demo1234';

const SB_URL = 'https://ugwspzniboazbovezwoo.supabase.co';
const SB_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVnd3Nwem5pYm9hemJvdmV6d29vIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA1NjAxNjcsImV4cCI6MjA5NjEzNjE2N30.ZQht9GAl33EZNf8n-oY-KX2ksHl05E2uK6oNlvdwDYo';
...
async function signIn() {
  ...
  if (error) { errEl.textContent = error.message; btn.textContent = 'Sign In'; btn.disabled = false; }
}
...
function getFiltered() {
  ...
  const trips    = state.trips.filter(t => inRange(t.trip_date) && (!f.type || t.trip_type === f.type) && (!f.status || t.status === f.status));
...
const COL = {
  income: 'rgba(22,160,106,.85)', incomeLine: '#16a06a',
  expense: 'rgba(229,96,75,.85)', expenseLine: '#e5604b',
  net: '#3b6cf6',
  delivery: 'rgba(59,130,246,.85)', oto: 'rgba(224,151,43,.9)',
  local: 'rgba(59,130,246,.85)', long: 'rgba(224,151,43,.9)',
  fuel: 'rgba(224,151,43,.9)', breakages: 'rgba(229,96,75,.85)',
  repairs: 'rgba(59,130,246,.85)', other: 'rgba(154,167,184,.85)',
...
function buildProjectionChart(trips, expenses) {
  ...
  if (keys.length < 1) { charts.proj = emptyChart(canvas,'Add trips to see a projection'); return; }
  ...
  const n = keys.length, lastIdx = n-1;
  ...
  const incProj = allKeys.map((_,i) => i < lastIdx ? null : (i===lastIdx ? incomeHist[lastIdx]  : round2(incForecast[i-n])));
  const expProj = allKeys.map((_,i) => i < lastIdx ? null : (i===lastIdx ? expenseHist[lastIdx] : round2(expForecast[i-n])));
...
function buildTripTypesChart(trips) {
  ...
  charts.tripTypes = new Chart(canvas, {
    type:'bar',
    data:{ labels:labels.map(monthLabel),
      datasets: types.map(ty => ({ label:ty, data:labels.map(l => byMonth[l][ty]), backgroundColor:colByType[ty], borderRadius:3, stack:'trips' })) },
    options:{ responsive:true, maintainAspectRatio:false,
      plugins:{ legend:{ position:'bottom', labels:{ boxWidth:12, padding:12 } } },
      scales:{ x:{ stacked:true, grid:{ color:GRID } }, y:{ stacked:true, beginAtZero:true, ticks:{ precision:0 }, grid:{ color:GRID } } } }
  });
}
...
function buildDistanceChart(trips) {
  ...
  charts.dist = new Chart(canvas, {
    type:'doughnut',
    data:{ labels:[`Local (${localN})`,`Long Distance (${longN})`],
...
async function submitTrip() {
  ...
  const dist   = parseFloat(document.getElementById('f-distance').value) || 0;
  const weight = parseFloat(document.getElementById('f-weight').value)   || 0;
  if (!type || !status || !date || !dist || !weight) { toast('Fill in all required fields (*)','error'); return; }
  const payout   = calcPayout();
  const tripData = {
    trip_date: date, trip_type: type, status,
    invoice_number: val('f-invoice') || null, trip_id: val('f-tripid') || null,
    distance_km: dist, weight_kg: weight, payout,
...
async function updateTripField(tripId, field, value) {
  ...
  if (field==='distance_km'||field==='weight_kg'||field==='trip_type') {
    ...
    if (trip) {
      ...
      payload.payout  = calcPayoutValue(merged.trip_type, parseFloat(merged.distance_km)||0, parseFloat(merged.weight_kg)||0);
    }
  }
...
function exportTripsExcel() {
  ...
  const rows = state.trips.map(t => ({
    Date: t.trip_date||'', Type: t.trip_type||'', Status: t.status||'',
    'Invoice #': t.invoice_number||'', 'Trip ID': t.trip_id||'',
    'Distance (km)': num(t.distance_km), 'Weight (kg)': num(t.weight_kg),
    'Long Distance': isLongDistance(t.distance_km||0)?'Yes':'No',
    'Payout (R)': round2(t.payout),
    'Grouped Invoices': t.grouped_invoices||'', 'Grouped Trip IDs': t.grouped_trip_ids||'',
    Notes: t.notes||'',
  }));
...
function exportExpensesExcel() {
  ...
  const rows = state.expenses.map(e => ({
    ...
    'Amount (R)': round2(e.amount), Description: e.description||'',
  }));
...
function exportSheet(rows, sheetName, filePrefix, totalCols={}) {
  ...
  ws['!cols'] = cols.map(c => ({ wch: Math.min(48, Math.max(c.length+2, ...rows.map(r=>String(r[c]??'').length+2))) }));
...
function renderSavedRanges(ranges) {
  ...
  container.innerHTML = ranges.map(r => `
    <button class="range-item" id="chip-${r.id}" onclick="applyRange('${r.start_date}','${r.end_date}','${r.id}')">
      ...
      <span class="ri-del" title="Delete range" onclick="deleteRange(event,'${r.id}')">Ὕ1</span>
    </button>`).join('');
}
...
function closeSaveRangeModal() { document.getElementById('save-range-modal').classList.remove('open'); }
